...
export default function AdminStablePage() {
  ...
  const toggleProduct = async (p: Product) => { try { await updateDocument(`products/${p.id}`, { active: p.active === false, updatedAt: new Date() }); await loadAll(); } catch (e) { setError(friendly((e as Error).message)); } };
  const removeProduct = async (p: Product) => { if (!window.confirm(`Ștergi produsul „${p.name}”?`)) return; try { await deleteDocument(`products/${p.id}`); await loadAll(); } catch (e) { setError(friendly((e as Error).message)); } };
